...
function onPathAction()
  ...
  elseif isPokemonUsable(falseswiper) then
		...
		elseif getMapName() == "Route 2" then
			pc = 0
			if jto == 0 then	
				moveToMap("Route 2 Stop") or moveToMap("Pewter City")
			elseif jto == 1 then
		...
		elseif getMapName() == "Route 10" then
			r = 1
			if x == 1 then
				return moveToMap("Rock Tunnel 1") or moveToCell(28,0)
			elseif x == 0 then
				if isNpcOnCell(9,9) then
					return talkToNpcOnCell(9,9) or moveToMap("Lavender Town") or moveToCell(28,0)
				end
			end
